...
namespace geometrydash {
  ...
  void PlayerManager::IsGameOver(Player &player, std::vector<Obstacle> &obstacles) {
    ...
    for (Obstacle &obstacle : obstacles) {
      if (((player.GetPosition().x + static_cast<float>(kPlayerWidth) / 2) >= (obstacle.GetPosition().x - 
                                     static_cast<float>(obstacle.GetWidth()) / 2)) && ((player.GetPosition().y + 
                                     static_cast<float>(kPlayerWidth) / 2) <= obstacle.GetPosition().y 
                                     && player.GetPosition().y >= obstacle.GetPosition().y - 
                                     static_cast<float>(obstacle.GetHeight()))) {
        game_over_ = true;
      }
    }
  }
